...
type AuthCallbackFunctionType = (body: any, client_id: string, scope: string, query: any) => any;

export default class SsoAuth2Server {
    private app: Express;

    constructor(redirectMode: boolean, port: number, route: string, session_secret: string, jwt_secret: string, authMethod: AuthCallbackFunctionType, requiredLoginParams: any) {
        this.app = express();
    ...
    start(){
        this.app.listen(EnvironmentCredentials.PORT, () => {
            console.info(`sso-server listening on port ${EnvironmentCredentials.PORT}`);
        });
    }
